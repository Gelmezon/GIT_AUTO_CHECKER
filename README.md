...
7. **Web 界面** — Vue3 前端（Figma 设计风格），支持登录、消息查看和管理后台
8. **管理员模式** — 内置 `superAdmin` 角色，可通过页面管理仓库、用户、任务并手动同步仓库
9. **多渠道通知** — 任务完成后通过企业微信 / Telegram / WhatsApp 推送摘要
10. **只读安全** — 系统只拉取和读取代码，**绝不 commit、push 或修改源码**
11. **MCP 服务** — 对外暴露标准化的 Git 只读操作（MCP 协议），供其他 AI 工具调用
...
│  ├─ API: /api/auth/* · /api/messages/* · /api/admin/*            │
...
如果你希望启用管理员后台，请至少配置 `[admin]` 中的 `email` 和 `password`。该账号不存储在 `users` 表中，登录后会直接进入 `/admin/dashboard`，可在页面里完成原本需要 `add-repo`、`add-task`、`add-user` 的操作。
...
你可以继续使用下面的 CLI 命令，也可以在配置好 `[admin]` 后，直接登录 `/admin/dashboard` 通过页面完成同样的操作。
...
系统支持内置 `superAdmin` 角色，用于把 CLI 管理动作迁移到 Web 后台：

- `superAdmin` 通过 `config.toml` 的 `[admin]` 配置登录，不需要提前执行 `add-user`
- 管理员账号不写入 `users` 表，因此不会出现在普通用户列表中
- 管理员登录后会进入 `/admin/dashboard`，而不是普通用户的 `/messages`
- 管理员可在页面中完成仓库管理、用户管理、任务管理，以及手动同步仓库
- 管理员接口统一挂在 `/api/admin/*` 下，必须使用 `role=superAdmin` 的 JWT 访问

建议：

- 生产环境务必修改默认管理员密码
- 如果部署在服务器上，建议限制 `config.toml` 文件权限
- 如需使用哈希密码，也可以将 `[admin].password` 配置为 bcrypt 哈希字符串
...
| 管理后台 | `/admin/dashboard` | `superAdmin` 仪表盘，查看仓库 / 用户 / 任务总览 |
| 项目管理 | `/admin/repos` | 创建、编辑、删除仓库，并支持手动同步 |
| 用户管理 | `/admin/users` | 创建、编辑、删除用户 |
| 任务管理 | `/admin/tasks` | 创建、筛选、删除任务 |
...
管理员接口要求登录账户为 `superAdmin`，否则会返回 `403`：

| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| GET | `/api/admin/dashboard` | 是（superAdmin） | 获取仪表盘统计和最近任务执行状态 |
| GET | `/api/admin/repos` | 是（superAdmin） | 获取仓库列表 |
| POST | `/api/admin/repos` | 是（superAdmin） | 新增仓库 |
| PUT | `/api/admin/repos/:id` | 是（superAdmin） | 编辑仓库 |
| DELETE | `/api/admin/repos/:id` | 是（superAdmin） | 删除仓库及其关联任务 |
| POST | `/api/admin/repos/:id/sync` | 是（superAdmin） | 手动克隆或拉取仓库 |
| GET | `/api/admin/users` | 是（superAdmin） | 获取用户列表 |
| POST | `/api/admin/users` | 是（superAdmin） | 新增用户 |
| PUT | `/api/admin/users/:id` | 是（superAdmin） | 编辑用户 |
| DELETE | `/api/admin/users/:id` | 是（superAdmin） | 删除用户 |
| GET | `/api/admin/tasks` | 是（superAdmin） | 获取任务列表，支持按状态筛选和分页 |
| POST | `/api/admin/tasks` | 是（superAdmin） | 新增任务 |
| DELETE | `/api/admin/tasks/:id` | 是（superAdmin） | 删除任务 |
...
如果你使用的是 `[admin]` 中配置的 `superAdmin` 账号，则登录后流程会变成：

```
1. 访问 http://127.0.0.1:3100/login
2. 输入 [admin] 中配置的 email / password
3. 登录成功后自动跳转到 /admin/dashboard
4. 在后台中完成：
   - /admin/repos：新增、编辑、删除、同步仓库
   - /admin/users：新增、编辑、删除用户
   - /admin/tasks：新增任务、按状态筛选、删除任务
```
...
如果你要登录的是管理员账号，则不要使用 `add-user`。请直接在 `config.toml` 中配置：
...
如果当前登录的是 `superAdmin`，这是正常行为。管理员账号不会进入消息中心，而是会跳转到 `/admin/dashboard`，并且不绑定普通用户消息列表。

**Q: 管理员账号如何创建？**

管理员账号不通过数据库创建，也不需要执行 `add-user`。它来自 `config.toml`：
...
**Q: 管理员登录后访问 `/messages` 会怎样？**

前端会把 `superAdmin` 自动引导回 `/admin/dashboard`。这是设计行为，因为管理员账号没有对应的 `user_id`，也不承载普通消息收件箱。
